...
{   
    ...
    // Test object count
    assert( oList.size() == JL_ARRAY_SIZE(pTestStrings) );
    ...
    {
        printf( "\tRemoving: %s\n", pTestStrings[i] );
        assert( oList.erase(pTestStrings[i]) );
    }
    ...
    {
        printf( "\tRemoving: %s\n", pTestStrings[i - 1] );
        assert( oList.erase(pTestStrings[i - 1]) );
    }
    ...
    for ( unsigned i = 0; i < 5; ++i )
    {
        ...
        for ( unsigned j = 0; j < nRemove; ++j )
        {
            ...
            const unsigned nSeek = rand() % oList.size();
